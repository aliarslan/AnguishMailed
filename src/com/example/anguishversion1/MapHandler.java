...
public class MapHandler {
	...
	private final GamePlayActivity parent;
	...
	public MapHandler(final GamePlayActivity pParent){
		this.parent = pParent;
	}
	...
	public void setDrawingMethod(){
		
	this.map.setIsometricDrawMethod(TMXIsometricConstants.DRAW_METHOD_ISOMETRIC_CULLING_TILED_SOURCE);
		
	}

	public void loadMap_Iso(){
		String location = "tmx/isometric_grass_and_water.tmx";
		final TMXLoader tmxLoader = new TMXLoader(this.parent.getAssets(), this.parent.getTextureManager(),TextureOptions.DEFAULT,this.parent.getVertexBufferObjectManager(), this.parent, null);
		TMXTiledMap txMap;
		try{
	...
	public void attachMap(TMXTiledMap pMap){
		...
		for (TMXLayer tmxLayer : layers) {
			...
			this.attachAllObjectLayers();     //Attach physics layer
	...
	public void attachPolyPointObject(final TMXObject pTMXObject){
		...
		for(int i = 0; i < pSceneCoordinates.length; i ++){
			...
			Line line = new Line(pX1, pY1, pX2, pY2, this.parent.getVertexBufferObjectManager());
			//final FixtureDef objectFixtureDef = PhysicsFactory.createFixtureDef(0.0f,0.0f, 0.0f);
			//mline = PhysicsFactory.createLineBody(this.parent.mPhysicsWorld,line,objectFixtureDef);
			line.setColor(this.parent.objectLines);
